...
export class AccountService {
  ...
  public registerWithThirdParty(model: RegisterWithExternal) {
    return this.httpClient.post<User>(`${environment.appUrl}/api/account/register-with-third-party`, model).pipe(
      map((user: User)=> {
        this.setUser(user);
      })
    )
  ...
  public loginWithThirdParty(model: LoginWithExternal) {
    return this.httpClient.post<User>(`${environment.appUrl}/api/account/login-with-third-party`, model)
      .pipe(
        map((user: User)=> {
          if(user){
            this.setUser(user);
